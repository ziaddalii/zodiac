...
export default function CartSidebar({locale}) {
    const {open, toggle_cart, cart_list, subtotal} = useCartList();
    ...
    return (
        <div className={`fixed overflow-hidden ${open ? "z-[200]" : "z-[-1]"} top-0 end-0`}>
            ...
            <Slide direction={`${locale === "ar" ? "right" : "left"}`} in={open}>
                ...
                <div
                    ref={cartRef}
                    className="shadow-lg h-[100dvh] overflow-auto flex flex-col justify-between p-4 bg-white transition-all top-0 duration-300 !z-[101] relative ease-in-out md:w-[25rem] w-[20rem]"
                >
                                ...
                                <div className="space-y-2 h-[60%]">
                                    ...
                                    >
                                        {cart_list.map((product) => (
                                            <div key={product.id}>
                                                <CartMenuCard product={product} locale={locale}/>
                                                <Divider/>
                                            </div>
                                        ))}
                                    </div>
                                ...
                                <div className="flex flex-col items-end justify-end flex-none bg-white">
                                    <div className="w-full h-fit bg-white">
                                        ...
                                        <div className="py-4 flex justify-between">
                                            <p className="text-lg font-bold">{t("fields.subtotal")}</p>
                                            <p className="font-bold">{t("fields.egp")}{format_price(subtotal)}</p>
                                        </div>
            ...
            <div
                onClick={() => toggle_cart(false)}
                className={`fixed top-0 left-0 w-full h-full bg-[#000000] pointer-events-none !z-[100] transition-all ${open ? "opacity-70 block pointer-events-auto" : "opacity-0 pointer-events-none"}`}
            ></div>
